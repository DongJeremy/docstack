...
为了理解这点，比如说我的域名 **`MindviewInc.com`**，将之反转并全部改为小写后就是 **`com.mindviewinc`**，这将作为我创建的类的独一无二的全局名称。（`com`、`edu`、`org`等扩展名之前在 Java 包中都是大写，但是 Java 2 之后都统一用小写。）我决定再创建一个名为 **simple** 的类库，从而细分名称：
